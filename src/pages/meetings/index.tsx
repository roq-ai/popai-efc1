...
function MeetingListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<MeetingInterface[]>(
    ...
    () =>
      getMeetings({
        relations: ['user_meeting_team_leader_idTouser', 'user_meeting_team_member_idTouser'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_meeting_team_leader_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_meeting_team_member_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_meeting_team_leader_idTouser?.id}`}>
                          {record.user_meeting_team_leader_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_meeting_team_member_idTouser?.id}`}>
                          {record.user_meeting_team_member_idTouser?.email}
                        </Link>
                      </Td>
                    )}
